...
# App code. alert_service and route_planner are separate top-level packages
# that road_viewer/server.py imports from directly (they're not nested under
# road_viewer/) -- without copying them too the image builds fine but the
# container crashes on startup with ModuleNotFoundError.
COPY road_viewer ./road_viewer
COPY alert_service ./alert_service
COPY route_planner ./route_planner
