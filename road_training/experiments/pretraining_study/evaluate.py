...
def evaluate_split(split):
    ...
    for arm, setting in plan['arms'].items():
        kind = PatchTST if setting['method'] == 'droppatch' else ArcTanEncoder
...
def freeze():
    ...
    for method, arms in [('droppatch', ('patchtst_scratch','droppatch_finetuned')),
                         ('arctan', ('dit_scratch','arctan_finetuned'))]:
        for seed in plan['seeds']:
