"""Computes both a pure-efficiency (fastest) route and a pothole-aware route over
a road graph, so a caller (or demo UI) can show the tradeoff explicitly rather
...
def _build_route_result(graph, nodes: List, edge_exposure: Dict[EdgeKey, float]) -> RouteResult:
    ...
    return RouteResult(nodes=nodes, coords=coords, distance_m=distance_m, duration_s=duration_s, pothole_exposure=exposure)
...
def find_route(
    ...
    'efficient' is the fastest route by real travel time, ignoring potholes entirely.
    ...
    efficient_nodes = nx.shortest_path(graph, origin_node, destination_node, weight="travel_time")
